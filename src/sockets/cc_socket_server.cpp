...
namespace lvc_tools
{
 ...
 /// Constructor
 /// ==================================================================
 CCSocketServer::CCSocketServer(std::string server_ID, std::string server_name,
 ...
 bool CCSocketServer::start()
 {
  ...
  inet_aton(Server_name.c_str(), &server_address.sin_addr);
  ...
  std::cout << "Socket SERVER [" << Server_ID << "] started and listening ..." << std::endl;
  ...
  std::cout << "Socket SERVER [" << Server_ID << "] connected" << std::endl;
 ...
 void CCSocketServer::stop()
 {
  ...
  std::cout << "Socket SERVER [" << Server_ID << "] stopped" << std::endl;
 ...
 /// ==================================================================
 int CCSocketServer::receive(unsigned char *buffer, const int size)
 {
  // Check that the server is connected with a client
  if (Status != SERVER_CONNECTED)
   {
 ...
 /// ==================================================================
 int CCSocketServer::send(void *buffer, const int n_send_bytes)
 {
  // Check that the server is connected with a client
  if (Status != SERVER_CONNECTED)
   {
  ...
  // Send data and get the number of sent data
  return ::write(FD_client, buffer, n_send_bytes);
 ...
 /// ==================================================================
 int CCSocketServer::receive(void *buffer, const int n_receive_bytes)
 {
  // Check that the server is connected with a client
  if (Status != SERVER_CONNECTED)
   {
  ...
  // Get the data and the number of received data
  return ::read(FD_client, buffer, n_receive_bytes);
